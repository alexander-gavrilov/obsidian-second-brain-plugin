...
**Decision:**
- Local present + global configured → ask: *"Migrate into the global vault (at `<global_path>`) or the local one (current directory)?"*
...
Import external notes into the Obsidian vault.
